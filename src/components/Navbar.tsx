    ...
    <>
      ...
      <header className="topbar">
        ...
        <nav className="desktop-nav" aria-label="Main navigation">
          {links
            .filter((x) => ["home", "projects", "experience"].includes(x.id))
            .map((link) => (
              <a
                key={link.id}
                href={"#" + link.id}
      ...
      {open && (
        ...
        >
          {[
            ...links,
            { id: "education", label: "Education", icon: FiLayers },
          ...
          >
            <FiPlay /> Replay Link Start
          </button>
        </nav>
      )}
      <aside className="side-rail">
        <span className="rail-index">
          {String(links.findIndex((link) => link.id === active) + 1).padStart(
            2,
            "0",
          )}{" "}
          / 05
        ...
        <button
          ...
          onClick={onReplay}
          aria-label="Replay Link Start"
        >
          <FiPlay />
        </button>
        <span className="rail-caption">EXPLORE THE INTERFACE</span>
      </aside>
    </>
  );
}
